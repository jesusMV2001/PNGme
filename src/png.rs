...
impl Png {
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        // Leer el archivo completo a un buffer
        let mut file = fs::File::open(path).map_err(|_| "No se pudo abrir el archivo")?;
    ...
    pub fn remove_first_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
            ...
            .position(|x| x.chunk_type().to_string() == chunk_type)
        {
            ...
            Err("Error al eliminar el primer chunk")
        }
    }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let mut result: Vec<u8> = Vec::with_capacity(&self.header.len() + &self.chunks.len());
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        //  Verificar longitud mínima (header)
        if bytes.len() < 8 {
            return Err("Bytes insuficientes para header");
        }

        //  Verificar que el header sea válido
        if bytes[..8] != Self::STANDARD_HEADER {
            return Err("STANDARD HEADER incorrecto");
        }

        //  Copiar el header
        let header: [u8; 8] = bytes[..8]
            .try_into()
            .map_err(|_| "No se pudo copiar el HEADER")?;

        // Preparar vector para chunks
        ...
        while index < bytes.len() {
            ...
            let desplazamiento = (12+length) as usize;
            let chunk = Chunk::try_from(&bytes[index..index + desplazamiento]);
            chunks.push(chunk?);
